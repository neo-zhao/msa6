...
public class Main {
	public static void main(String[] args) {
		...
		// filling in missing parameters for the bloons (the starting position, path,
		// and path number); also handles multiple paths (if there are any)
		...
		while (!eventList.isEmpty()) {
			// jump to next event time (find the smallest double in the eventList and update
			// eventList)

			// finding the smallest and removing it from the eventList
			double elapsedTime = Double.MAX_VALUE;
			...
			Set<Double> updatedEvents = new HashSet<Double>();
			for (double t : eventList) {
